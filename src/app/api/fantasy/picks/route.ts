...
export const FANTASY_BUFFS = [
  ...
  { id: "lucky_loser", name: "Неудачник?", icon: "🍀", min: 0, max: 0, desc: "Не дает бонусов к очкам (поэтому и называется «Неудачник?»), но полностью отменяет любые штрафы за высокий скилл: Саппорт получает 100% очков, а Лошадка не штрафуется" }
];
