...
export default function Home() {
  ...
  const startDraftSetup = async () => {
    ...
    // Randomize captain order directly so Team 1 is 1st pick, Team 2 is 2nd pick, etc.
    const shuffledCaptains = [...draftCaptains] as [string, string, string, string];
    for (let i = shuffledCaptains.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [shuffledCaptains[i], shuffledCaptains[j]] = [shuffledCaptains[j], shuffledCaptains[i]];
    }
          ...
          <div style={{ width: "100%" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem", width: "100%" }}>
              ...
              {activeTab === 'fantasy' && (() => {
                ...
                const tournamentParticipantNames = Array.from(new Set([
                ...
                const handleRandomSquad = () => {
                  const poolToUse = fantasyPlayerPool;
                  if (!poolToUse || poolToUse.length < 3) return;
                            ...
                            <div style={{
                              ...
                              <div>
                                <select
                                  ...
                                  onChange={e => {
                                    const val = e.target.value;
                                    const found = fantasyPlayerPool.find(p => p.nickname === val || p.playerId === val);
